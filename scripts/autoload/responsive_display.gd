...
const ORIENTATION_GUARD_SCENE: PackedScene = preload(
	"res://scenes/shared/system/orientation_guard.tscn"
)
...
func _build_orientation_guard() -> void:
	var guard_node := ORIENTATION_GUARD_SCENE.instantiate()
	_orientation_layer = guard_node as CanvasLayer

	if _orientation_layer == null:
		push_error("OrientationGuard scene root harus CanvasLayer.")
		return

	add_child(_orientation_layer)
	_orientation_blocker = _orientation_layer.get_node(
		"OrientationBlocker"
	) as ColorRect
	_orientation_label = _orientation_layer.get_node(
		"OrientationBlocker/Center/Panel/Margin/Column/OrientationMessage"
	) as Label
	_orientation_button = _orientation_layer.get_node(
		"OrientationBlocker/Center/Panel/Margin/Column/OrientationButton"
	) as Button

	var panel := _orientation_layer.get_node(
		"OrientationBlocker/Center/Panel"
	) as PanelContainer
	var margin := _orientation_layer.get_node(
		"OrientationBlocker/Center/Panel/Margin"
	) as MarginContainer
	var column := _orientation_layer.get_node(
		"OrientationBlocker/Center/Panel/Margin/Column"
	) as VBoxContainer
	var title := _orientation_layer.get_node(
		"OrientationBlocker/Center/Panel/Margin/Column/Title"
	) as Label

	if (
		_orientation_blocker == null
		or _orientation_label == null
		or _orientation_button == null
		or panel == null
		or margin == null
		or column == null
		or title == null
	):
		push_error("OrientationGuard scene-authored structure tidak lengkap.")
		return
	...
	_orientation_button.custom_minimum_size = Vector2(
		0.0,
