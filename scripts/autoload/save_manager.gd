...
func load_v3_history_records() -> Array:
	var records: Array = []
	var history_dir: DirAccess = DirAccess.open(
		_v3_path("history")
	)
	...
	for file_name_value in files:
		...
		var payload: Dictionary = _read_json(
			_v3_path("history/" + file_name)
		)

		if payload.is_empty():
			push_warning(
				"History schema v3 tidak dapat dibaca untuk tampilan: "
				+ file_name
			)
			continue

		var history_value: Variant = payload.get(
			"history",
			null
		)
		...
		if not history_value is Dictionary:
			push_warning(
				"History schema v3 tidak memiliki blok history: "
				+ file_name
			)
			continue
		...
		if id_history.is_empty():
			push_warning(
				"History schema v3 tanpa id_history: "
				+ file_name
			)
			continue
